...
{
	_a456 = level.drones.drone_spawners;
	_k456 = getFirstArrayKey( _a456 );
	while ( isDefined( _k456 ) )
	{
		s_data = _a456[ _k456 ];
		...
		_k456 = getNextArrayKey( _a456, _k456 );
	}
	return undefined;
}
...
drone_spawn( team, offset, distance_down_path, drones )
{
	...
	while ( isDefined( drones.dr_player_trace ) && drones.dr_player_trace )
	{
	...
	guy = spawn( "script_model", bullettrace( spawnpos, spawnpos + vectorScale( ( 1, 0, 0 ), 100000 ), 0, self )[ "position" ] );
...
spawnpoint_playersview()
{
	...
	while ( i < players.size )
	{
		...
		{
			success = bullettracepassed( players[ i ] geteye(), self.origin + vectorScale( ( 1, 0, 0 ), 48 ), 0, self );
			if ( success )
			{
				player_view_count++;
			}
...
drone_think( firstnode, override_target_node )
{
	...
	{
		self.turrettarget = spawn( "script_origin", self.origin + vectorScale( ( 1, 0, 0 ), 50 ) );
...
drone_dodeath( deathanim, deathremovenotify )
{
	...
	if ( tracedeath )
	{
		...
		endanimationlocation = physicstrace( endanimationlocation + vectorScale( ( 1, 0, 0 ), 128 ), endanimationlocation - vectorScale( ( 1, 0, 0 ), 128 ) );
		...
		else
		{
			...
			relativeoffset = vectorScale( ( 1, 0, 0 ), 50 );
			...
			secondpos = physicstrace( secondpos + vectorScale( ( 1, 0, 0 ), 128 ), secondpos - vectorScale( ( 1, 0, 0 ), 128 ) );
	...
	{
		self moveto( self.origin - vectorScale( ( 1, 0, 0 ), 100 ), 7 );
		wait 3;
	}
	if ( !isDefined( self ) )
	{
...
drone_runchain( point_start )
{
	...
	while ( isDefined( self ) )
	{
		...
		{
			if ( !isDefined( point_start.a_targeted ) && !isDefined( point_start.script_delete ) )
			{
				self.dontdelete = undefined;
		...
		else point_end = point_start.a_targeted;
		...
		else
		{
			...
			self.v_destination = bullettrace( point_end[ index ].origin, point_end[ index ].origin + vectorScale( ( 1, 0, 0 ), 100000 ), 0, self )[ "position" ];
...
#/
							...
							endpos = physicstrace( endpos + vectorScale( ( 1, 0, 0 ), 64 ), endpos - ( 0, 0, level.drones.trace_height ) );
...
drone_shoot_bullets( e_target )
{
	...
	v_tag_flash = self.origin + vectorScale( ( 1, 0, 0 ), 50 );
...
drone_cover_fire( type )
{
	...
	while ( 1 )
	{
		...
		rightvec = anglesToRight( self.angles );
		upvec = anglesToUp( self.angles );
		relativeoffset = vectorScale( ( 1, 0, 0 ), 300 );
...
{
	array = [];
	while ( isDefined( level.drones.team ) )
	{
		if ( str_team == "axis" )
		{
			while ( isDefined( level.drones.team[ "axis" ] ) )
			{
		...
		{
			while ( isDefined( level.drones.team[ "allies" ] ) )
			{
...
drones_delete_spawned( str_noteworthy )
{
	...
	_a4487 = a_m_drones;
	index = getFirstArrayKey( _a4487 );
	while ( isDefined( index ) )
	{
		m_drone = _a4487[ index ];
		...
		index = getNextArrayKey( _a4487, index );
	}
}
