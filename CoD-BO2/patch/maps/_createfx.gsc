...
/#
	...
	while ( i < level.createfxent.size )
	{
		...
		if ( ent.textalpha > 0 )
		{
			...
			print3d( ent.v[ "origin" ] + printright + vectorScale( ( 1, 1, 1 ), 15 ), ent.v[ "fxid" ], colors[ ent.v[ "type" ] ][ colorindex ], ent.textalpha, scale, print_frames );
		}
		i += print_frames;
	}
	if ( isDefined( highlightedent ) )
	{
		...
		if ( highlightedent.textalpha > 0 )
		{
			...
			print3d( highlightedent.v[ "origin" ] + printright + vectorScale( ( 1, 1, 1 ), 15 ), highlightedent.v[ "fxid" ], colors[ highlightedent.v[ "type" ] ][ colorindex ], highlightedent.textalpha, scale, 1 );
#/
		}
	}
}
...
delete_spawns()
{
	...
	while ( isDefined( _k4279 ) )
	{
		...
		_a4283 = spawns;
		_k4283 = getFirstArrayKey( _a4283 );
		while ( isDefined( _k4283 ) )
		{
			spawn = _a4283[ _k4283 ];
			spawn delete();
			_k4283 = getNextArrayKey( _a4283, _k4283 );
		}
