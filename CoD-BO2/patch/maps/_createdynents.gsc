...
/#
	...
	i = 0;
	while ( i < destructible_defs.size )
	{
...
/#
	...
	while ( 1 )
	{
		...
		{
			while ( current_num >= ( hud_array.size - 1 ) )
			{
				continue;
			}
			current_num++;
...
/#
	...
	while ( 1 )
	{
		...
		{
			if ( key == "downarrow" || key == "dpad_down" )
			{
				level.spray[ "power" ] -= 50;
			...
			else
			{
				...
				{
					if ( key == "rightarrow" || key == "dpad_right" )
					{
						level.spray[ "rate" ] += 0,05;
					...
					{
						if ( key == "button_lshldr" || key == "button_rshldr" )
						{
							if ( isDefined( level.selected_group ) )
							{
								level.spray[ "model" ] = get_random_model();
...
/#
	...
	for ( ;; )
	{
		...
		else while ( isDefined( no_mod_button ) && level.debug_player buttonpressed( no_mod_button ) )
		{
			wait 0,05;
		}
		while ( use_attackbutton && level.debug_player attackbuttonpressed() )
		{
...
/#
	...
	while ( i < objects.size )
	{
		...
		ent = objects[ i ];
		difference = vectornormalize( ent.origin - ( level.debug_player.origin + vectorScale( ( 1, 0, 0 ), 55 ) ) );
	...
	while ( isDefined( highlighted_object ) )
	{
...
/#
	...
	i = 0;
	while ( i < level.models[ type ].size )
	{
...
/#
	level endon( "stop_createdynents" );
	selected_delete( 1 );
	level.unselected_color = ( 1, 0, 0 );
	level.selected_color = ( 1, 0, 0 );
