...
#/
	...
	while ( isDefined( _k244 ) )
	{
		...
		spawn_location.origin += vectorScale( ( 0, 0, 1 ), 65 );
...
conditional_power_indicators()
{
	...
	{
		if ( self.is_current_ball_location )
		{
			self setclientfield( "turn_active_perk_light_red", 0 );
...
machine_think()
{
	...
	while ( 1 )
	{
		...
		level notify( "pmmove" );
		while ( player.score < level._random_zombie_perk_cost )
		{
		...
		else
		{
			...
			while ( 1 )
			{
				...
				else self.bottle_spawn_location setmodel( model );
