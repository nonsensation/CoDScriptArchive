...
main()
{
	...
	for(i=0;i<vehicles.size;i++)
	{
		...
		if(isdefined(vehicles[i].script_team) && vehicles[i].script_team == "allies" && debugvehicles == "off")
		{
			...
			{
				vehicles[i] maps\_truck_gmi::init();
				vehicles[i] thread maps\_truck_gmi::handle_attached_guys();
			}
			if(vehicles[i].vehicletype == "GermanBMW")
			{
				vehicles[i] maps\_bmwbike_gmi::init();
				vehicles[i] thread maps\_bmwbike_gmi::handle_attached_guys();
			}
		}
	...
	for(i=0;i<spawntrigs.size;i++)
	{
		...
		if(isdefined(targ))
		{
			maps\_vehiclespawn_gmi::spawner_setup(targ);
		}
...
//	evspawncount = 0;
	...
	for(i=0;i<vehicles.size;i++)
	{
		...
		if(isdefined (vehicles[i].targetname))
		{
			...
			{
				maps\_vehiclespawn_gmi::spawner_setup(vehicles[i]);
			}
			else
			if(isdefined(vehicles[i].script_team) && vehicles[i].script_team == "allies" && debugvehicles == "off")
			{
				...
				{
					vehicles[i] maps\_kubelwagon_gmi::init();
					vehicles[i] thread maps\_kubelwagon_gmi::handle_attached_guys();
				}
			}
		}
	}
...
delete_group()
{
	...
	stopattachedfx(self);
	wait 1;  // waitting 1 second because stop attachedfx is wierd.
	self delete();
}
...
trigger_vehiclegroupspawn()
{
	...
	if(!isdefined(self.target))
		println("no target on trigger at ",self.origin);	
	vehicle = spawnvehiclegroup(self.target);
	if(isdefined(self.script_noteworthy))
	{
		level notify (self.script_noteworthy, vehicle);
	}	
	level notify ("vehiclespawned",vehicle);		
}

spawnvehiclegroup(vehicletargetname)
{
	vehicle = maps\_vehiclespawn_gmi::vehicle_spawn(vehicletargetname);
	if(vehicle.vehicletype == "HalfTrack")  // this is inconsistant with the rest of the stuff because the Halftrack script is a bit different.
	{
		vehicle.deathmodel = "xmodel/v_ge_lnd_halftrack";
		vehicle.health = 3000;
		vehicle.guys = [];
		vehicle thread maps\_halftrack_gmi::init();
		vehicle.ismoving = 0;
		vehicle getonpath();
		return vehicle; 
...
{
	if(isdefined(self.script_noteworthy) && self.script_noteworthy == "runtoplayer")
	{
		self waittill ("trigger");
		thread enemy_groupruntoplayer();
	...
	else
	{
		...
		if(!isdefined(node))
			return; // hack.. 7/13/2004 10:57AM I deleted a node in sicily2 and don't want to recompile to get rid of a script error. hack to be removed apon next compile. 
		thread enemy_grouprunto(node);
	}
}
enemy_groupruntoplayer()
{
	if(!isdefined(self.script_vehiclegroup))
	{
	...
	for(j=0;j<ai.size;j++)
	{
		if(isdefined(ai[j].script_vehiclegroup) && ai[j].script_vehiclegroup == self.script_vehiclegroup)
		{
			ai[j] setgoalentity (level.player);
			if(isdefined(self.radius))
			{
				ai[j].goalradius = self.radius; 
			}
			ai[j].goalradius = 850; 
		}
	}
...
//	thread debug_vehiclespeed();
	...
	if(isdefined(self.script_delay) && !(isdefined(self.script_noteworthy) && self.script_noteworthy == "nodelay"))
	{
...
//	guy thread drawme();
	...
	guy maps\_utility_gmi::cant_shoot();
...
//				self thread draw_crash_path();
		...
		if(isdefined(pathpoints[i].script_noteworthy))
		{
			...
			{
				self notify("shoot_guns");
/*
				if (level.script == "bomber")		//This checks for bomber map
				{
					self thread maps\_bf109_gmi::shoot_guns();
				}
				else
				{
					self thread maps\_stuka_gmi::shoot_guns();
				}
*/
			}
			else
			if (pathpoints[i].script_noteworthy == "stop_firing")
			{
				...
				//self thread maps\_stuka_gmi::turret_kill();
			}
			else
			...
			if (pathpoints[i].script_noteworthy == "drop_bombs") // MikeD: For dropping all bombs.
			{
				if(isdefined(pathpoints[i].script_delay))
				{
					wait pathpoints[i].script_delay;
				}
				self notify ("drop_bombs",undefined,undefined,1);
				//self thread maps\_stuka_gmi::drop_bombs(undefined, undefined, 1);
			}
			else
			if (pathpoints[i].script_noteworthy == "drop_1_bomb") // MikeD: For dropping 1 bomb.
			{
				if(isdefined(pathpoints[i].script_delay))
				{
					wait pathpoints[i].script_delay;
				...
				self notify ("drop_bombs",1,undefined,1);
//				self thread maps\_stuka_gmi::drop_bombs(1, undefined, 1);
			}
			else
			if (pathpoints[i].script_noteworthy == "speed_0")
			{
				self notify("set_speed",0);
			}
			else
			if (pathpoints[i].script_noteworthy == "speed_90") 
			{
				self notify("set_speed",90);
			}
			else
			if (pathpoints[i].script_noteworthy == "speed_180") 
			{
				self notify("set_speed",180);
			}
			else
			if (pathpoints[i].script_noteworthy == "speed_max")
			{
				self notify("set_speed",999);	//	i just pass 999 to signify max 
			}
			else
			{
				self notify ("groupedanimevent",pathpoints[i].script_noteworthy);
			}
		}
		waited = false;
	}
	self waittill ("reached_end_node");
	self notify ("skidoff");  //nothing should ever skid when it's at it's end node!
}
...
player_vehicle_paths()
{
	...
	while(isdefined (pathpoint))
	{
		if(isdefined(pathpoint.script_noteworthy))
		{
			...
			else if (pathpoint.script_noteworthy == "skidoff")
			{
				self setWaitNode(pathpoint);
				self waittill ("reached_wait_node");
				self notify ("skidoff");
			...
			// used so far
			// skidon - turns on skid sound for sharp corners
			// skidonoff - turns of skid sound
			// idle - plays whatever idle animation is on the driver
			// fire - starts firing sequence
			// turnleft - bmwbikedriver turns left
			// turnright - bmwbikedriver turns right
			// turnhardleft - bmwbikedriver turns hardleft
			else
			{
				self setWaitNode(pathpoint);
				self waittill ("reached_wait_node");
				self notify ("groupedanimevent",pathpoint.script_noteworthy);
			}
