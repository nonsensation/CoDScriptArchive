...
quecheck()
{
	...
	{
		if(isdefined(level.playertank) && self.enemyque[0] == level.playertank)
...
kill()
{
	...
	self clearTurretTarget();
	println("playfxontag ",self.deathfx);
...
queremove(target)
{
	...
	self.enemyque = maps\_utility_gmi::array_remove(self.enemyque,target);
//	thread queupdate(self.enemyque);
