...
main()
{
	...
	for (i=0;i<other_mg42s.size;i++)
		mg42s = maps\_utility_gmi::add_to_array ( mg42s, other_mg42s[i] );

 	maps\_utility_gmi::array_thread(mg42s, ::mg42_think);
...
mg42_think ()
{
	...
	while (1)
	{
		...
		((!isdefined (ai[i].script_mg42)) || (ai[i].script_mg42 != self.script_mg42)))
			excluders = maps\_utility_gmi::add_to_array ( excluders, ai[i] );

		ai = maps\_utility_gmi::getClosestAI (node.origin, undefined, excluders);
		excluders = undefined;
...
grenade_trigger (trigger)	// fix fix
{
	...
	println ("^3 Touchers ", touchTriggers.size);
 	...
 	maps\_utility_gmi::array_thread(spawners, ::grenade_think, delay);
 	maps\_utility_gmi::array_thread(touchTriggers, ::grenade_trigger_think, delay);
	...
	while (count > 0)
	{
		...
		while (level.player istouching (trigger))
		{
			...
			maps\_utility_gmi::array_notify(spawners, "throw");
		 	maps\_utility_gmi::array_notify(touchTriggers, "throw");
		 	println ("^3 Throw!");

			// Allow the designer the ability to change the delay time before spawning a new grenade.
			if(isdefined(trigger.script_delay_min) && isdefined(trigger.script_delay_max))
			{
				range = trigger.script_delay_max - trigger.script_delay_min;
				delay = (trigger.script_delay_min + randomfloat(range));
//				println("Grenade Tossing is waiting: ",delay);
				wait delay;
			}
			else
			{
...
grenade_trigger_think (delay)
{
	...
	while (1)
	{
		...
		if (isalive (thrower))
		{
			...
			velocity = maps\_utility_gmi::vectorScale (velocity, distance (org.origin, dest.origin)*2);
...
{
	spawners = getentarray (trigger.target,"targetname");
	maps\_utility_gmi::array_thread(spawners, ::flood_prethink);
	trigger waittill ("trigger");
	for (i=0;i<spawners.size;i++)
		if (isdefined (spawners[i]))
			final_spawners = maps\_utility_gmi::add_to_array (final_spawners, spawners[i]);

	if (isdefined (final_spawners))
		maps\_utility_gmi::array_thread(final_spawners, ::flood_think, trigger);
...
flood_spawn (spawners)
{
	...
	maps\_utility_gmi::array_thread(spawners, ::flood_prethink);
	maps\_utility_gmi::array_thread(spawners, ::flood_think);
}
...
flood_think(trigger)
{
	...
	if ((isdefined (self.spawnflags)) && (!(self.spawnflags & 1)))
		maps\_utility_gmi::error ("Spawner at origin " + self.origin + "/" + (self getorigin()) + " is not a spawner");
	...
	while (self.count > 0)
	{
		...
		if(	isdefined(self.script_stalingradspawn) 
			&& self.script_stalingradspawn )
		{	
...
spawner_mg42_notify (num)
{
	...
	if ((isdefined(mg42s[i].script_mg42)) && (mg42s[i].script_mg42 == num))
		mg42 = maps\_utility_gmi::add_to_array (mg42, mg42s[i]);
...
{
	hackBlendTime = 0.2;
	// Hack to make these guys not do the basepose
	wait 0.1;
	if (!isalive (self))
	{
		return;
	}
	/#[[anim.println]]("Doing special prone spawn hack, time: ",GetTime(), ", pose: ",pose);#/
	if (self isStanceAllowed("stand"))
	{
		self setAnimKnobAll (%stand_alert_1, %root, 1, hackBlendTime, 1);
		self.anim_idleSet = "a";		
		self.anim_pose = "stand";
	}
	else if (self isStanceAllowed("crouch"))
	{
		self setAnimKnobAll (%crouch_alert_A_idle, %root, 1, hackBlendTime, 1);
		self.anim_idleSet = "a";
		self.anim_pose = "crouch";
	}
	else if (self isStanceAllowed("prone"))
	{
		self setAnimKnobAll (%prone_aim_idle, %root, 1, hackBlendTime, 1);
	...
	{
		println ("FixBasePose: no stance allowed for ent ",self GetEntNum(),"!  Remaining standing.");
		self setAnimKnobAll (%stand_alert_1, %root, 1, hackBlendTime, 1);
		self.anim_idleSet = "a";		
		self.anim_pose = "stand";
	}
	// Now trick PlayTransitionAnimation into thinking that its animation is done.
	wait (hackBlendTime);
	self notify("transAnimDone", "end");
	self notify("transAnimDone2", "end");
}

/*triggerBasePoseSpawner ( trigger )
{
	spawners = getentarray (trigger.target,"targetname");
	trigger.target = "turned off for base pose guys";
	trigger waittill ("trigger");
	for (i=0;i<spawners.size;i++)
	{
		spawners[i] fixBasePose();
		spawners[i] dospawn();
	}
}*/
...
{
//	if (isdefined (spawner.script_fixbasepose))
//		maps\_utility_gmi::array_levelthread(getentarray (spawner.targetname,"target"), ::triggerBasePoseSpawner);
...
//			maps\_utility_gmi::error ("FORCESPAWN");
		...
		if(isdefined(level.kharkov2_missionfail_tracker) && level.kharkov2_missionfail_tracker)
		{
			level thread [[level.Turret_Missionfail_Tracker]]();
		}	
...
spawn_think_action (spawned, targetname)
{
	if (isdefined (spawned.script_fixbasepose))
		spawned thread fixBasePoseThread();

	if (isdefined (spawned.script_breathpuff))
		spawned thread maps\_breath_gmi::breath_fx_init();
		
	self thread tanksquish();
//	maps\_spawner_gmi::waitframe(); // silly way to let the drop health get insured if a spawner is placed to go off at start
	...
	if ((spawned.team == "allies") && (!isdefined (spawned.script_personality)))
		spawned thread maps\_personality_gmi::create();
...
//				println ("new targetname is ", targetname);
	...
	if (!override)
	{
		...
		{
			if ( (getcvar("g_gameskill") == "2") || (getcvar("g_gameskill") == "3") )
			{
				num = randomint(3);
				if (num == 0)
					spawned.grenadeawareness  = .2;
				else
					spawned.grenadeawareness  = .5;
			}
			else
			{
				num = randomint(3);
...
//		println ("giving someone some grenades.");
	...
	// MikeD: Added 03/30/04
	// Enable dontdropweapon, I guess, it used to be dontdropweapon, but change to DropWeapon in code...
	// This fixes it for SPAWNABLE AI. Not for AI already (non-spawned) in map.
	if(isdefined(spawned.dontdropweapon))
	{
	...
	{
		spawned thread maps\_patrol_gmi::patrol();
		return;
	}
	...
	// If using _respawner_gmi:: stop this function from continueing.
...
go_to_node(node)
{
	...
	if (!isdefined (node))
	{
		...
		// MikeD: Self.script_random, allows for the AI to randomly choose a node
		if(isdefined(self.script_random))
		{
			if(node.size > 1)
			{
				num = randomint(node.size);
				println("^2node size ",node.size);
				node = node[num];
				println("^2Random Node ", num);
			}
			else
			{
				node = node[0];
...
use_a_turret (turret)
{
	...
	(getent (turret.target,"targetname").classname == "script_origin"))
	{
		...
		self thread maps\_mg42_gmi::manual_think (turret);
		if (isdefined (self.script_mg42auto))
			println ("AI at origin ", self.origin , " has script_mg42auto");
	}

	self thread maps\_mg42_gmi::mg42_firing(turret);
...
fallback_spawner_think (num, node)
{
	...
	while (self.count > 0)
	{
		...
		maps\_spawner_gmi::waitframe(); // Wait until he does all his usual spawned logic so he will run to his node
...
newfallback_overmind (num, group)
{
	...
	for (i=0;i<nodes.size;i++)
	{
		if ((isdefined (nodes[i].script_fallback)) && (nodes[i].script_fallback == num))
			fallback_nodes = maps\_utility_gmi::add_to_array (fallback_nodes, nodes[i]);
	}
	...
	for (i=0;i<ai.size;i++)
	{
		if (((isdefined (ai[i].script_fallback)) && (ai[i].script_fallback == num)) ||
			...
			fallback_ai = maps\_utility_gmi::add_to_array (fallback_ai, ai[i]);
	}
	ai = undefined;
...
{
	// MikeD: Since this was made for Dawnville, let's return 
	// if the level is not dawnville.
...
friendly_wave (trigger)
{
	...
	if (trigger.targetname == "friendly_wave")
	{
		...
		if (assert)
			maps\_utility_gmi::error ("Look above");
	}
...
friendly_wave_masterthread()
{
	...
	maps\_utility_gmi::array_thread(triggers, ::set_spawncount, 0);

	friends = getaiarray ("allies");
	maps\_utility_gmi::array_thread(friends, ::friendlydeath_thread);
...
{
	if (!isdefined (trigger.target))
		maps\_utility_gmi::error ("No target for friendly_mg42 trigger, origin:" + trigger getorigin());

	node = getnode (trigger.target,"targetname");

	if (!isdefined (node.target))
		maps\_utility_gmi::error ("No mg42 for friendly_mg42 trigger's node, origin: " + node.origin);
...
friendly_mg42_wait_for_use(mg42)
{
	mg42 endon ("friendly_finished_using_mg42");
	self.useable = true;
	self setHintString(&"CGAME_USEAIONMG42");
	self waittill ("trigger");
...
//	self thread maps\_mg42_gmi::mg42_firing(mg42);
...
killfriends_missionfail_think(ai, istank, team)
{
	...
	if ( (isdefined (istank)) && (istank == 1) )
	{
		...
		if (team == "allies")
		{
			if ( (attacker == level.player) || ( (level.script == "pavlov") && (attacker.classname == "misc_turret") ) )
			{
				if ( (isdefined (ai.failWhenKilled)) && (ai.failWhenKilled == true) )
					thread killfriends_missionfail();
				else if (level.killedaxis < 4)// If the player hasn't killed at least 4 enemies before killing this friendly
					thread killfriends_missionfail();
				else
				{
					level.ffpoints = 0;
		...
		else if (team == "axis")
		{
			...
			if ( (level.script == "pavlov") && (attacker.classname == "misc_turret") )
				level.killedaxis++;
		}
	}
	else
...
{
	if(getcvar("mike_ff_off") == 1) // MikeD: So I can test killing team respawning.
	{
		println("Mike CHEAT! No FriendlyFire");
		return;
	}
	...
	missionfailed();// Fail the mission for killing friendlies
}
...
println("^1(_spawner_gmi::killfriends_damage_think): Friendly Fire: DMG: ",damage,"^1 level.ffpoints: ",level.ffpoints, " ^1level.killedaxis: ", level.killedaxis);
