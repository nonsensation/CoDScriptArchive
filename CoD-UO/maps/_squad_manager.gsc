...
manage_spawners(strSquadName,mincount,maxcount,ender,spawntime, spawnfunction)
{
	...
	{
		maps\_utility_gmi::error("SQUAD MANAGER:  Could not find spawners for squad " + strSquadName);
		return;
	}
	if ( squad_targets.size == 0 )
	{
		maps\_utility_gmi::error("SQUAD MANAGER:  Could not find target nodes for squad " + strSquadName);
		return;
	}
	...
	while(1)
	{
		...
		if (aSquad.size < mincount)
		{
			while ( aSquad.size < maxcount )
			{
				...
				if(	isdefined(squad_spawn[spawn_index].script_stalingradspawn) 
					&& squad_spawn[spawn_index].script_stalingradspawn )
				{	
