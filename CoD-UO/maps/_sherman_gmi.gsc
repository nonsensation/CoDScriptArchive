...
main()
{
	precachemodel("xmodel/v_us_lnd_sherman_d");
	precachevehicle("ShermanTank");
	precacheturret("mg30cal_sherman_tank");
	loadfx ("fx/explosions/vehicles/sherman_complete.efx");
...
init(optionalMachinegun)
{
	...
	if (isDefined(optionalMachinegun) )
		...
		thread maps\_tank_gmi::turret_attack_think("mg30cal_sherman_tank");
...
player_kill()
{
	...
	if(self.model == "xmodel/v_us_lnd_sherman")
		self.deathmodel = "xmodel/v_us_lnd_sherman_d";
	else
	...
	self.deathfx    = loadfx ("fx/explosions/vehicles/sherman_complete.efx");
...
// any problems with the tanks HERE is the place to look 
...
//	tank2 thread force_anim(4,%c_us_sherman_idle1);

force_anim(tag,animname)
{
	if (!isdefined(self.guys))
		return;

	
	for(u=0;u<self.guys.size;u++)
	{
		bloke = self.guys[u];
		if (bloke.climb_tag == tag)
		{
			bloke stopanimscripted();
			bloke.animname = bloke.original_animname;	
			bloke notify("stopanim");
			bloke maps\_anim_gmi::anim_single_solo(bloke, animname, bloke.exittag, undefined, self);
			bloke.animname = "sherman_tank";
			bloke thread maps\_anim_gmi::anim_loop_solo(bloke, ("idle" + bloke.climb_tag), bloke.exittag, "stopanim", undefined, self);
			return;
		}
	}
}
...
sherman_tank_anim(tank, angle_offset)
{
	...
	self thread maps\_anim_gmi::anim_loop_solo(self, ("idle" + self.climb_tag), self.exittag, "stopanim", undefined, tank);
