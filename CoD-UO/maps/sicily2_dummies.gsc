...
main()
{
	precacheModel("xmodel/sicily2_dummies_seagulls_a");
	precacheModel("xmodel/m_seagull_a");
}
...
dummies_path_think(path_anim, anim_wait )
{
	...
	for (i=0;i<self.total_tags;i++)
	{
		...
		dummy setmodel ("xmodel/m_seagull_a");
...
dummy_think( dummies_path, anim_wait )
{
	...
	self thread modelloop(%m_seagull_fly_a);
...
modelloop(modelanim)
{
	while(1)
	{
		self setFlaggedAnimKnobRestart("animdone", modelanim);
