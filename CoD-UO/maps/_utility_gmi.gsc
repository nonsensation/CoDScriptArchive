...
make_player_feel_important()
	Thread on AI. AI starts to take real damage after player shoot him. Can stop at anytime by sending 
	notify of "stop player feels important".
*/
...
autosave(num)
{
	savedescription =  maps\_autosave_gmi::getnames(num);
	...
	maps\_autosave_gmi::healthchecksave(num, savedescription, imagename);
...
//	if (self.team == "axis")
//		maps\_utility_gmi::error ("AXIS DID CANT SHOOT");
...
get_vehicle_group()
{
	...
	for (i=0;i<spawners.size;i++)
	{
		...
		&& (spawners[i].script_vehiclegroup == self.script_vehiclegroup) )
			vehiclegroup = maps\_utility_gmi::add_to_array ( vehiclegroup, spawners[i] );
	}
	return vehiclegroup;
}
...
cannon_effect ( source )
{
	...
	level thread maps\_fx_gmi::OneShotfx(source.script_fxid, source.origin, source.script_delay, org);
}
...
//	loopfx(				fxId,	fxPos, 	waittime,	fxPos2,	fxStart,	fxStop,	timeout)
	maps\_fx_gmi::loopfx(	firefx,	org1,	delay,		org,	undefined,	ender,	timeout);
}
...
brush_show()
{
	...
	if(isdefined(self.script_noteworthy) && self.script_noteworthy == "notsolid")
	{
		self notsolid();
	}
	else
...
//	temp_vec = maps\_utility_gmi::vectorScale (temp_vec, 250 + randomint (100));
...
flood_spawn (spawners)
{
	maps\_spawner_gmi::flood_spawn (spawners);
}
...
set_ambient (track)
{
	...
	{
		level.global_ambient_org = spawn("script_origin",level.player.origin);
		level.global_ambient_org.targetname = "global_ambient";
		level.global_ambient_org playloopsound(level.global_ambient[track]);
		println("^2Playing GLOBAL AMBIENT: ", track);
	}
}
...
{	
	self endon ("death");
	self endon ("stop player feels important");

	if (!isdefined (oldhealth))
		oldhealth = self.health;
		
	self thread stop_make_player_feel_important(oldhealth);
...
stop_make_player_feel_important(newhealth)
{
	self endon ("death");
	self waittill ("stop player feels important");
	self.health = newhealth;
}

blend_upanddown_sound(ent,timeout,low,high)
{
	blend = spawn( "sound_blend",(0,0,0) );
	blend.origin = ent.origin;
	forward = anglestoforward(ent.angles);

	volume = 0;
	volspeed = 0.02;
	blend_off_clock = gettime() + (timeout * 1000);
	while(1)
	{
		if(getCvar("scr_debug_blender") == "1")
		{
			print3d(blend.origin,high + " " + volume + " " + forward);
		}

		if ((volume>=0) && (volume<=1))
		{
//			println("set sound blend");
			blend setSoundBlend( low, high, volume );
		}
		if ((volume>=1) && (volspeed!=0))
		{
//			println("stop fade up");
			volspeed = 0;
		}

//		println("clock ",blend_off_clock," - ",gettime());


		if ((blend_off_clock < gettime()) && (volspeed>=0))
		{
//			println("fade");
			volspeed = -0.002;
		}
		if ((volume<=0) && (volspeed<0))
		{
//			println("done");
			return;
		}
//		println("vol ",volume,"speed ",volspeed);
		volume = volume + volspeed;
		wait (0.05);
	}
}
