...
attach_bombs(bomb_count)
{
	...
	for(i=0;i<self.bomb_count;i++)
	{
		...
		{
			self.bomb[i] linkto(self,"tag_smallbomb0" + (i - 1) + "Right",(0,0,-4),(-10,0,0));
		}
	}
}
...
drop_bombs(amount, delay, delay_trace, damage_range, max_damage, min_damage, trace_dist)
{
	...
	if(isdefined(amount))
	{
		...
		for(i=0;i<amount;i++)
		{
			...
			self.bomb[i] thread bomb_trace(delay_trace, damage_range, max_damage, min_damage, trace_dist);
...
bomb_trace(delay_trace, damage_range, max_damage, min_damage, trace_dist)
{
	self endon("death");
...
//		println("Dist ", dist);
		...
		{
			if (!isdefined(level.stuka_bomb))
			{
				maps\_utility_gmi::error ("level.stuka_bomb is not defined. Please specify in your script.");
			}
			else
			{
...
{
	if(isdefined(level.stuka_bomb_qpower))
	{
		quake_power = level.stuka_bomb_qpower;
	}
	else
	{
		quake_power = 0.5;
	...
	if(isdefined(level.stuka_bomb_qtime))
	{
		quake_time = level.stuka_bomb_qtime;
	}
	else
	{
		quake_time = 2;
	...
	if(isdefined(level.stuka_bomb_qradius))
	{
		quake_radius = level.stuka_bomb_qradius;
	}
	else
	{
		quake_radius = 1024;
	...
	playfx ( level.stuka_bomb, self.origin );
