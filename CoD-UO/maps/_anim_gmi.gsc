...
anim_loop ( guy, anime, tag, ender, node, tag_entity )
{
	...
	while (1)
	{
		...
		for (i=0;i<guy.size;i++)
		{
			...
			//guy[i] endon("death");
			if(isai(guy[i]) && !isalive(guy[i]))
			{
				println("^1ANIM LOOP RETURNS HERE GUY "+guy[i] getEntityNumber());
				return;
			}
...
anim_single (guy, anime, tag, node, tag_entity)
{
//	println (guy[0].animname, " ^5doing animation ", anime);
	if (isdefined (tag))
	{
		if (isdefined (tag_entity))
		{
...
//			println (guy[i].animname , " facialanim ", facialanim);
			if (doAnimation)
				...
				guy[i] animscripts\face::SaySpecificDialogue(facialAnim, dialogue, 1.0, "single dialogue");
...
//		println ("MOODY ------------------------------");
//	println ("^4GUY: ", guy.animname,", ANIMATION: ", anime);
	while (1)
	{
...
//				playfxOnTag (level._effect["pistol"], guy, "tag_flash");
				...
				{
					guy thread maps\_utility_gmi::playSoundOnTag (level.scr_notetrack[guy.animname][i]["sound"]);
...
{
	if (!isdefined (guy.ScriptModel))
		maps\_utility_gmi::Error ("Tried to remove a model with delete model before it was create model'd on guy: " + guy.animname);
	...
	for (i=0;i<guy.ScriptModel.size;i++)
	{
		if (isdefined (array["explosion"]))
		{
			...
			forward = maps\_utility_gmi::vectorScale (forward, 120);
...
// MikeD: Added EXPLODER support so instead of just playing an effect, we can use the exploder scripts. 
// Currerntly used in Noville.
		else if (isdefined (array["exploder"]))
		{
			level thread maps\_utility_gmi::exploder(array["exploder"]);
		}
...
printloops (guy, anime)
{
	...
	guy.loops++;
	if (guy.loops > 1)
		maps\_utility_gmi::error ("guy with name "+ guy.animname+ " has "+ guy.loops+ " looping animations played, anime: "+ anime);
}
