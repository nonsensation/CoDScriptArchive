...
//	positions[3]["idleoccurrence"][1] = 200;
...
//	positions[2]["standidleoccurrence"][2] = 500;
	...
	positions[3]["sittag"] = "tag_guy01"; // back left
...
guy_idle(guy)
{
	...
	while(1)
	{
		...
		println("^3...",guy.sittag," IS PLAYING ",guy.idle[0]);
		guy notify ("idle");
//		theanim = randomoccurrance(guy,guy.idleoccurrence);
		animontag(guy,guy.sittag,guy.idle[0]);
	}
}
...
attach_guys(name,driver)
{
	...
	if(!isdefined(guys[0]))
		guys = undefined;
	self thread maps\_kubelwagon_gmi::handle_attached_guys(guys);
}
...
kill()
{
	fx = loadfx("fx/explosions/vehicles/kubelwagon_complete.efx");
...
//THAD Added this for sicily1 kubelwagen event, feel free to use it
#using_animtree ("generic_human");
handle_attached_guys2(guys)
{
	level.guyevent = [];
...
//	positions[2]["getout"] = %wagon_backleft_jumpout;
	...
	positions[0]["jumpin"] = %c_kubel_driver_jumpin;
	positions[1]["jumpin"] = %c_kubel_passenger_jumpin;
	positions[2]["jumpin"] = %c_kubel_guy1_jumpin;
	positions[3]["jumpin"] = %c_kubel_guy2_jumpin;

	positions[0]["idle"][0] = %c_kubel_driver_idle;
	positions[1]["idle"][0] = %c_kubel_passenger_idle;
	positions[2]["idle"][0] = %c_kubel_guy1_idle;
	positions[3]["idle"][0] = %c_kubel_guy2_idle;
...
//	positions[0]["sittag"] = "tag_driver";
	...
	//Switching this to "tag_body
//	climbnode [0] = "tag_climbnode";
	climbnode [0] = "tag_body";

	climbanim [0] = %c_kubel_driver_jumpin;
	climbanim [1] = %c_kubel_passenger_jumpin;
	climbanim [2] = %c_kubel_guy1_jumpin;
	climbanim [3] = %c_kubel_guy2_jumpin;
...
//	movetospotanim = %stand_alert_1;
	...
	while(self.health > 0)
	{
		...
		for(i=0;i<guysarray.size;i++)
		{
			guysarray[i].jumpin = positions[pos]["jumpin"];
			guysarray[i] thread kubelwagen_allies_jumpin(guysarray);
			
			guysarray[i] thread drawmypos(pos);
		...
		println("^5WE ARE ALL IN THE KUBEL!");
...
//			thread guy_waitjumpout(guy);
			...
			println("^3---variable set for ", guy.animname);
			...
			println("^3$$$linked guy ", guy.animname);
			...
			if(pos != 0)
			{
				...
				println("^3&&&Guy teleport ", guy.animname);
...
kubelwagen_allies_jumpin(guys)
{
	self endon ("death");

	println(self.animname, " ^3ABOUT TO ANIMATE");
	//when the guy is at the node make him play the animation and get into the truck
	self animscripted("climbin done", level.kubelwagen getTagOrigin("tag_body"), level.kubelwagen getTagAngles("tag_body"), self.jumpin);
	self waittillmatch ("climbin done", "end");
	println(self.animname, "^3 IS DONE ANIMATING");

	self linkto (level.kubelwagen, "tag_body", (0, 0, 0), (1, 1, 1));

	level.guysin++;

	count = 0;
	for(n=0;n<guys.size;n++)
	{
		if(isalive(guys[n]))count++;
	}

	if(count == level.guysin) level notify("guys in kubelwagen");
}
...
//	while(1)
//	{
//		print3d(self.origin + (0,0,100), pos, (1,0,0),1);
//		
//		if(isdefined(self.sittag))
//		{
//		print3d(self.origin + (0,0,150), self.sittag, (1,1,1),1);
//		}
//		wait 0.05;
//	}
}
