main()
{
	...
	if ( !(isdefined (level.mortar) ) )
		maps\_utility_gmi::error ("level.mortar not defined. define in level script");
}

tankdrive_kursk_style()
{
	// Mortars never land within 400 units of a tank
	if ( !(isdefined (level.mortar) ) )
		maps\_utility_gmi::error ("level.mortar not defined. define in level script");
	mortartrigs = getentarray ("mortartrigger","targetname");
	for(i=0;i<mortartrigs.size;i++)
	{
		mortars = getentarray (mortartrigs[i].target,"targetname");
		for (j=0;j<mortars.size;j++)
			mortars[j].has_terrain = false;
		
		//mortartrigs[i].mortargroup = 0;
		mortartrigs[i] thread script_mortargroup_mortarzone_avoidtanks();
	}
	/*
	while(1)
	{
		...
		mortartrig thread script_mortargroup_mortarzone_avoidtanks();
		lasttrig = mortartrig;
	}
	*/
}
...
script_mortargroup_mortarzone()
{
	...
	while(level.mortarzone == self.script_mortargroup && isdefined(self.groupedmortars))
	{
		...
		self.groupedmortars = maps\_utility_gmi::array_remove(self.groupedmortars,self.groupedmortars[pick]);
		delay = randomFloat( 2.4 ) + 4;
		wait( delay );
	}
}

script_mortargroup_mortarzone_avoidtanks()
{
	mortars = getentarray (self.target,"targetname");
	while(1)
	{
		self waittill ("trigger");
		while (1)
		{
			pick = randomint(mortars.size);  // - 1 ?
			dist = 1000;
			for (i=0;i<level.tanks.size;i++)
			{
				if (distance(level.tanks[i].origin, mortars[pick].origin) <= 400)
					break;
			}
			break;
		}
		
		mortars[pick] thread script_mortargroup_domortar();
		//self.groupedmortars = maps\_utility_gmi::array_remove(self.groupedmortars,self.groupedmortars[pick]);
		delay = randomFloat( 2.4 ) + 4;
		wait( delay );
	}
