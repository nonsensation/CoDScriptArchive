...
CrouchRunToProne()
{
	...
	/#[[anim.assert]]((self.anim_movement == "run"||self.anim_movement == "walk"), "SetPoseMovement::CrouchRunToProne "+self.anim_movement);#/
...
PlayTransitionAnimation2(transAnim, endPose, endMovement, endAiming, useCriticalSection, finalAnim)
{
	...
	// Play the anim
	/#[[anim.println]]("PlayTransitionAnimation2 is playing ",transAnim);#/
...
//			println("Entering critical section. "+self.anim_script+": "+self.anim_pose+" to "+endPose+", "+self.anim_movement+" to "+endMovement);
	...
	// Play the anim
	/#[[anim.println]]("PlayTransitionAnimation is playing ",transAnim);#/
	self setflaggedanimknoball("transAnimDone2", transAnim, %body, 1, .05, 1);
	if (!isDefined(self.anim_pose))
		self.pose = "undefined";
	if (!isDefined(self.anim_movement))
		self.movement = "undefined";
