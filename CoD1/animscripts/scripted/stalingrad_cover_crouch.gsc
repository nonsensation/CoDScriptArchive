...
main()
{
	...
	/#[[anim.println]] ("starting stalingrad_cover_crouch");#/
	self animMode ("angle deltas");	// Stops movement deltas from taking effect, so that he'll stay on his node.
									// TODO - this doesn't work if I immediately go into a critical section, which I currently do.
	...
	if (isDefined(self.customnode))
	{
		...
		/#[[anim.println]] ("Stalingrad_cover_crouch: custom node is type "+nodeType+".");#/
	}
	else
	{
		/#[[anim.println]] ("Stalingrad_cover_crouch: No node specified for custom animscript.");#/
	...
	else 
	{
		...
		else
		{
			if (isAI(self))	// Is this necessary?
			{
				...
				self [[anim.SetPoseMovement]](pose,"stop");
			}
			else
			{
		...
		for (;;)
		{
			/#[[anim.println]]("stalingrad_cover_crouch: main loop start");#/
			...
			{
				/#[[anim.println]]("stalingrad_cover_crouch: preparing to shoot");#/
//				thread animscripts\utility::drawDebugLine(self GetEye(), GetEnemyPos(), (1,0,0), 4);
				...
				/#[[anim.assert]] (self.anim_movement == "stop", "stalingrad_cover_crouch: About to call aim, movement is "+self.anim_movement);#/
				...
				/#[[anim.println]]("stalingrad_cover_crouch: shooting");#/
				animscripts\combat::ShootVolley(0, "forceShoot", self.customtarget );
                self notify ("killHackTrackCustomTarget");
				/#[[anim.locspam]]("scc5");#/
			}
			/#[[anim.println]]("stalingrad_cover_crouch: shooting finished");#/
...
//			if ( !isDefined(self.customFastAttack) || (self.CustomFastAttack==false) )
			...
			{
				/#[[anim.println]]("stalingrad_cover_crouch: ducking");#/
				...
				/#[[anim.println]]("stalingrad_cover_crouch: ducking finished");#/
			}
			else
			{
...
duckBetweenShots()
{
	...
	case "crouch":
		if (self.anim_special == "cover_crouch")
		{
			...
			{
				self.anim_special = "none";
				animscripts\SetPoseMovement::PlayTransitionAnimation(%hideLowWall2crouch, "crouch", "stop", 1, 0);
				self.anim_idleSet = "b";
				self.anim_special = "cover_crouch";
				animscripts\SetPoseMovement::PlayTransitionAnimation(%crouch_aim2hideLowWallb, "crouch", "stop", 0, 0);
			}
			else if (self.anim_idleSet == "c")
			{
				self.anim_idleSet = "b";
				animscripts\SetPoseMovement::PlayTransitionAnimation(%hideLowWallc2hideLowWallb, "crouch", "stop", 0, 0);
			}
		...
		else
		{
			self.anim_special = "cover_crouch";
			self.anim_idleSet = "b";
			animscripts\SetPoseMovement::PlayTransitionAnimation2(%crouch_aim2hideLowWallb, "crouch", "stop", 0, 0);	// TODO Set anim_special & idleSet here.
		}
		break;
	}
}
...
idle_a(nodeAngle)
{
	...
	/#[[anim.println]]("  Exiting idle_a");#/
}

idle_b()
{
	/#[[anim.println]]("  Entering idle_b");#/
	choice = randomint(100);
	playbackRate = 0.9 + randomfloat(0.2);
	...
	self animscripts\shared::DoNoteTracks("animdone");
	/#[[anim.println]]("  Exiting idle_b");#/
}

idle_c(nodeAngle)
{
	/#[[anim.println]]("  Entering idle_c");#/
	self OrientMode( "face angle", nodeAngle );
