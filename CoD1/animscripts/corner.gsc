...
CornerAttack( cornerAngle, corneranims , changeReason )
{
	...
	if (animscripts\weaponList::usingAutomaticWeapon())
	{
		...
		if ( animscripts\utility::sightCheckNode() )
		{
			...
			self thread DoAutoFire("cornerattack autofire done", numShots, anim.blindAccuracyMult[self.team]);	// Poor accuracy until we "look".
		...
		if ( !self isSuppressed() && animscripts\utility::sightCheckNode() )
		{
			...
			estimatedTime =  0.1 * (float)numShots / (float)animRate;
			self thread animscripts\combat::KillRunawayAutofire("cornerattack autofire done", estimatedTime);
			self thread DoAutoFire("cornerattack autofire done", numShots, 1);
			self waittill("cornerattack autofire done");
		}
		/#[[anim.locspam]]("ca5c");#/
	}
...
DoAutofire(notifystring, numshots, accuracy)
{
